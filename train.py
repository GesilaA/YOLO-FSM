...
# from model.yolov4 import Yolov4 as Yolov3
# from model.yolov5 import Yolov5 as Yolov3
...
class Trainer(object):
    ...
    def __load_model_weights(self, weight_path, resume):
        if resume:
            ...
            if self.whichNet == 'yolov3':
                self.yolov3.load_state_dict(chkpt['model'])
            elif self.whichNet == 'yolov4':
                chkpt = {'_Yolov4__backnone.' + k: v for k, v in chkpt.items() if k[:8] == 'backbone'}
                self.yolov3.load_state_dict(chkpt, strict=False)
            elif self.whichNet == 'yolov5':
                chkpt = {KEY_MAP[k]: v for k, v in chkpt.items() if k in KEY_MAP.keys()}
                self.yolov3.load_state_dict(chkpt, strict=False)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes)  in enumerate(self.train_dataloader):
                ...
                if i%10==0:
                    s = ('Epoch:[ %d | %d ]    Batch:[ %d | '
                         '%d ]    loss_giou: %.4f    loss_conf: %.4f    loss_cls: %.4f    loss: %.4f    '
            ...
            if epoch >= 0:
                ...
                with torch.no_grad():
                    APs, _, _ = Evaluator(self.yolov3).APs_voc()
                    for i in APs:
                         print("{}\t-->\tmAP: {}".
                               format(i, APs[i]))
                         mAP += APs[i]
