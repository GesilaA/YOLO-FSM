...
def dviou_xyxy_numpy(boxes1, boxes2):
    """
    :param boxes1: boxes1和boxes2的shape可以不相同，但是需要满足广播机制
    :param boxes2: 且需要保证最后一维为坐标维，以及坐标的存储结构为(xmin, ymin, xmax, ymax)
    :return: 返回boxes1和boxes2的IOU，IOU的shape为boxes1和boxes2广播后的shape[:-1]
...
def RIOU_xywh_torch(boxes1, boxes2):
    '''
    计算ciou = iou - p2/c2 - av
    :param boxes1: (8, 13, 13, 3, 4)   pred_xywh
    :param boxes2: (8, 13, 13, 3, 4)   label_xywh
    ...
    # 两矩形中心点距离的平方
    p2 = torch.pow(boxes1[..., 0] - boxes2[..., 0], 2) + torch.pow(boxes1[..., 1] - boxes2[..., 1], 2) / 2
    # 增加av。加上除0保护防止nan。
    # atan1 = torch.atan(boxes1[..., 2] / (boxes1[..., 3] + 1e-9))
    # atan2 = torch.atan(boxes2[..., 2] / (boxes2[..., 3] + 1e-9))
    wh2 = (torch.pow(boxes1[..., 2] - boxes2[..., 2], 2) + torch.pow(boxes1[..., 3] - boxes2[..., 3], 2)) / 2
    # v = 4.0 * torch.pow(atan1 - atan2, 2) / (math.pi ** 2)
    # a = v / (1 - iou + v)
    wh2[iou <= 1e-6] = 0
    # riou = - 1. * ((1.0 - iou) * p2 + wh2) / enclose_c2
    riou = iou - (1.0 * p2 / enclose_c2 + wh2 / enclose_c2)
...
def dnms(bboxes, score_threshold, iou_threshold, sigma=0.3, method='nms'):
    """
    :param bboxes:
    假设有N个bbox的score大于score_threshold，那么bboxes的shape为(N, 6)，存储格式为(xmin, ymin, xmax, ymax, score, class)
    其中(xmin, ymin, xmax, ymax)的大小都是相对于输入原图的，score = conf * prob，class是bbox所属类别的索引号
    ...
    for cls in classes_in_img:
        ...
        while len(cls_bboxes) > 0:
            ...
            if method == 'nms':
                diou_mask = diou > iou_threshold
                weight[diou_mask] = 0.0
            if method == 'soft-nms':
                weight = np.exp(-(1.0 * diou ** 2 / sigma))
            cls_bboxes[:, 4] = cls_bboxes[:, 4] * weight
            score_mask = cls_bboxes[:, 4] > score_threshold
            cls_bboxes = cls_bboxes[score_mask]
    return np.array(best_bboxes)
...
def plot_box(bboxes, img, id = None, color=None, line_thickness=None):
    ...
    # cv2.waitKey(0)
    img = cv2.cvtColor(img* 255.0, cv2.COLOR_RGB2BGR)
