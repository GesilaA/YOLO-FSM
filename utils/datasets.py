...
class VocDataset(Dataset):
    def __init__(self, anno_file_type, img_size=416, mode=None):
        self.img_size = img_size  # For Multi-training
        self.classes = cfg.DATA["CLASSES"]
        self.num_classes = len(self.classes)
        self.class_to_id = dict(zip(self.classes, range(self.num_classes)))
        self.__annotations = self.__load_annotations(anno_file_type, mode=mode)
        self.call_num = 0
        self.rand_list = [408, 512, 320] * 10000
        self.rewrite_c = [0, 0, 0]
    ...
    def __load_annotations(self, anno_type, mode):
        ...
        if anno_type == 'train':
            if mode is not None:
                ...
                anno_path = os.path.join('./data', anno_type+"_annotation_"+mode+".txt")
    ...
    def __parse_annotation(self, annotation):
        ...
        # img, bboxes = dataAug.RandomGenImg(p=.2)(np.copy(img), np.copy(bboxes))
        # img, bboxes = dataAug.ColorChange()(np.copy(img), np.copy(bboxes))
        img, bboxes = dataAug.RandomHorizontalFilp()(np.copy(img), np.copy(bboxes))
        img, bboxes = dataAug.RandomCrop()(np.copy(img), np.copy(bboxes))
        img, bboxes = dataAug.RandomAffine()(np.copy(img), np.copy(bboxes))
        # img, bboxes = dataAug.Resize((self.rand_list[self.call_num//6], self.rand_list[self.call_num//6]), True)(np.copy(img), np.copy(bboxes))
    ...
    def __creat_label(self, bboxes):
        ...
        for bbox in bboxes:
            ...
            for i in range(3):
                ...
                # if sum(iou_mask) > 1:
                #     iou_mask = iou_scale == max(iou_scale)

                if np.any(iou_mask):
                    xind, yind = np.floor(bbox_xywh_scaled[i, 0:2]).astype(np.int32)
                    # Bug : 当多个bbox对应同一个anchor时，默认将该anchor分配给最后一个bbox

                    # iou_mask_temp = [False, False, False]
                    # if np.array(label[i][yind, xind, iou_mask, 4:5]) == 1.0:
                    #     for a in range(3):
                    #         if label[i][yind, xind, a, 4:5] != 1.0:
                    #             iou_mask_temp[a] = True
                    #             break
                    # if any(iou_mask_temp):
                    #     iou_mask = iou_mask_temp

                    self.rewrite_c[i] += np.sum(np.array(label[i][yind, xind, iou_mask, 4:5]) == 1.0)
            ...
            if not exist_positive:
                ...
                self.rewrite_c[best_detect] += np.sum(np.array(label[best_detect][yind, xind, best_anchor, 4:5]) == 1.0)
...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(voc_dataset, shuffle=True, batch_size=16, num_workers=0)

    for i, (img, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes) in enumerate(dataloader):
        print('batch: %d' % i)
        print("REWRITE: ", voc_dataset.rewrite_c)
        # if img.shape[0] == 1:
