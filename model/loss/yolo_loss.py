...
class YoloV3LosswithFeatureModule(nn.Module):
    def __init__(self, anchors, strides, iou_threshold_loss=0.5):
        super(YoloV3LosswithFeatureModule, self).__init__()
        self.__iou_threshold_loss = iou_threshold_loss
        self.__strides = strides

    def forward(self, p, p_d, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes,
                m_label, sm_feature, l_label, ml_feature):
        strides = self.__strides
        ...
        loss_l, loss_l_giou, loss_l_conf, loss_l_cls = self.__cal_loss_per_layer(p[2], p_d[2], label_lbbox,
                                                               lbboxes, strides[2])
        loss_sm = self.__calc_feature_loss(m_label, sm_feature)
        loss_ml = self.__calc_feature_loss(l_label, ml_feature)

        loss = loss_l + loss_m + loss_ml + loss_s + loss_sm
        loss_giou = loss_s_giou + loss_m_giou + loss_l_giou
        loss_conf = loss_s_conf + loss_m_conf + loss_l_conf
        loss_cls = loss_s_cls + loss_m_cls + loss_l_cls
    ...
    def __cal_loss_per_layer(self, p, p_d, label, bboxes, stride):
        ...
        riou= tools.RIOU_xywh_torch(p_d_xywh, label_xywh).unsqueeze(-1)
        ...
        l_g = (torch.sum(loss_giou)) / batch_size
        l_c = (torch.sum(loss_conf)) / batch_size
        l_s  = (torch.sum(loss_cls)) / batch_size
        loss = loss_giou + loss_conf + loss_cls
        lt = (torch.sum(loss_giou, -1) + torch.sum(loss_conf, -1) + torch.sum(loss_cls, -1)) / batch_size

        return loss, l_g, l_c, l_s

    def __calc_feature_loss(self, label_feature, out_feature, alpha=.5, T=1):
        feature_loss = nn.KLDivLoss()(F.log_softmax(out_feature / T, dim=1),
                                      F.softmax(label_feature / T, dim=1)) \
                       * (alpha * T * T) + F.cross_entropy(out_feature, label_feature) * (1. - alpha)
        return feature_loss
