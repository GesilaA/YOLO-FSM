...
class Evaluator(object):
    def __init__(self, model, visiual=True, metric_root=None):
        self.classes = cfg.DATA["CLASSES"]
        self.pred_result_path = os.path.join('data', 'results')
        # self.val_data_path = os.path.join(cfg.DATA_PATH[3:], 'VOCtrainval-2007', 'VOCdevkit', 'VOC2007')
        self.val_data_path = os.path.join(cfg.DATA_PATH[3:], 'VOCtest-2007', 'VOCdevkit', 'VOC2007')
    ...
    def APs_voc(self, multi_test=False, flip_test=False):
        # img_inds_file = os.path.join(self.val_data_path,  'ImageSets', 'Main', 'trainval.txt')
        ...
        for img_ind in tqdm(img_inds):
            ...
            if self.metric_root is not None:
                ff = open(os.path.join(self.metric_root, img_ind+'.txt'), 'wt')

            if bboxes_prd.shape[0]!=0 and self.__visiual and self.__visual_imgs < 1000:
                boxes = bboxes_prd[..., :4]
            ...
            for bbox in bboxes_prd:
                ...
                if self.metric_root is not None:
                    ...
                    ff.writelines('%s %f %d %d %d %d\n' % (class_name, float(score), x1, y1, x2, y2))
    ...
    def __calc_APs(self, iou_thresh=0.5, use_07_metric=True):
        """
        计算每个类别的ap值
        :param iou_thresh:
        :param use_07_metric:
        ...
        for i, cls in enumerate(self.classes):
            R, P, AP, scaled_AP = voc_eval.voc_eval(filename, annopath, imagesetfile, cls, cachedir, iou_thresh, use_07_metric)
            #F1 = 2 * P * R / (P + R)
            APs[cls] = AP
            #F1s[cls] = F1
            scaled_APs[cls] = scaled_AP
        if os.path.exists(cachedir):
            shutil.rmtree(cachedir)

        return APs, F1s, scaled_APs
